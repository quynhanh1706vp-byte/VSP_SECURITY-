#!/usr/bin/env bash
# VSP — fedramp_cmmc_sbom.sh
# FedRAMP/CMMC compliance + SBOM tab + Dashboard v5
set -e
echo ">>> FedRAMP + CMMC + SBOM + Dashboard v5"
mkdir -p static internal/api/handler

cat > 'static/index.html' << 'VSP_DASH_V5'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
  ...
  <div class="main">
    ...
    <div id="panel-sbom" class="panel">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        <div>
          <div style="font-size:20px;font-weight:700;color:var(--text-1)">Software Bill of Materials</div>
          <div style="font-size:13px;color:var(--text-2);margin-top:4px">CycloneDX 1.5 format — download per scan run</div>
        </div>
      </div>
      <div class="card" style="margin-bottom:20px">
        <div style="font-size:13px;color:var(--text-2);margin-bottom:12px;font-weight:600">Recent runs with SBOM available</div>
        <div class="table-wrap">
          <table id="sbom-table">
            <thead><tr>
              <th>RID</th><th>Mode</th><th>Gate</th><th>Findings</th><th>Created</th><th>SBOM</th>
            </tr></thead>
      ...
      <div class="card" style="background:var(--accent-dim);border-color:var(--accent)">
        <div style="font-size:13px;font-weight:600;color:var(--accent);margin-bottom:8px">CycloneDX format includes:</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;color:var(--text-2)">
          <span>✓ BOM format: CycloneDX 1.5</span>
          <span>✓ All vulnerability IDs (rule IDs)</span>
          <span>✓ Severity ratings per finding</span>
          <span>✓ Tool attribution per vuln</span>
          <span>✓ Fix signals / remediation URLs</span>
          <span>✓ Scan metadata & timestamp</span>
        </div>
      </div>
    </div>
    ...
    <div id="panel-compliance2" class="panel">
      <div style="display:flex;gap:12px;margin-bottom:20px">
        <button class="btn btn-primary btn-sm" onclick="loadFedRAMP()" id="btn-fedramp">FedRAMP Moderate</button>
        <button class="btn btn-outline btn-sm" onclick="loadCMMC()" id="btn-cmmc">CMMC Level 2</button>
      </div>
      ...
      <div class="grid4" style="margin-bottom:20px">
        <div class="card" style="grid-column:span 4">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
            <span id="compliance2-title" style="font-weight:600;font-size:15px;color:var(--text-1)">FedRAMP Moderate</span>
            <span id="compliance2-coverage" style="font-size:24px;font-weight:700;color:var(--green)">—%</span>
          </div>
          <div style="background:var(--border);border-radius:4px;height:8px;overflow:hidden">
            <div id="compliance2-bar" style="height:8px;background:var(--green);border-radius:4px;width:0%;transition:width .5s"></div>
          </div>
          <div style="display:flex;justify-content:space-between;margin-top:8px;font-size:11px;color:var(--text-3)">
            <span id="compliance2-assessed">0 assessed</span>
            <span id="compliance2-total">0 total</span>
          </div>
        </div>
      </div>

      <!-- Controls/Practices table -->
      <div class="card">
        <div class="table-wrap">
          <table id="compliance2-table">
            <thead><tr>
              <th>ID</th><th>Family/Domain</th><th>Control/Practice</th><th>Tools</th><th>Status</th>
            </tr></thead>
...
function showPanel(name, btn) {
  ...
  if (name === 'compliance2') loadFedRAMP()
...
async function loadSBOM() {
  try {
    ...
    document.getElementById('sbom-tbody').innerHTML = runs.map(r => `
      <tr>
        <td style="font-family:monospace;font-size:11px;color:var(--accent)">${r.rid.slice(-12)}</td>
        <td><span class="pill pill-queued">${r.mode}</span></td>
        <td>${r.gate ? `<span class="pill pill-${r.gate.toLowerCase()}">${r.gate}</span>` : '—'}</td>
        <td style="color:${(r.total||0)>0?'var(--red)':'var(--green)'}">${r.total||0}</td>
        <td style="font-size:11px;color:var(--text-3)">${new Date(r.created_at).toLocaleString()}</td>
        <td>
          <a href="/api/v1/sbom/${r.rid}" 
             onclick="downloadSBOM(event,'${r.rid}')"
...
function downloadSBOM(e, rid) {
  e.preventDefault()
  const a = document.createElement('a')
  a.href = `/api/v1/sbom/${rid}`
  a.download = `sbom-${rid.slice(-8)}.cdx.json`
...
async function loadFedRAMP() {
  document.getElementById('btn-fedramp').className = 'btn btn-primary btn-sm'
  document.getElementById('btn-cmmc').className = 'btn btn-outline btn-sm'
  try {
    const data = await api('GET', '/compliance/fedramp')
    renderCompliance2(data, 'FedRAMP Moderate', data.controls, 'id', 'family', 'title')
  } catch(e) { console.error('fedramp', e) }
}

async function loadCMMC() {
  document.getElementById('btn-fedramp').className = 'btn btn-outline btn-sm'
  document.getElementById('btn-cmmc').className = 'btn btn-primary btn-sm'
  try {
    const data = await api('GET', '/compliance/cmmc')
    renderCompliance2(data, 'CMMC Level 2', data.practices, 'id', 'domain', 'practice')
  } catch(e) { console.error('cmmc', e) }
}

function renderCompliance2(data, title, items, idKey, familyKey, titleKey) {
  ...
  document.getElementById('compliance2-bar').style.background =
  ...
  document.getElementById('compliance2-total').textContent =
    (data.total_controls || data.total_practices || 0) + ' total controls'

  document.getElementById('compliance2-tbody').innerHTML = (items||[]).map(c => `
    <tr>
      <td><code style="font-size:11px;color:var(--purple)">${c[idKey]}</code></td>
      <td style="font-size:12px;color:var(--text-2)">${c[familyKey]}</td>
      <td style="font-size:12px">${c[titleKey]}</td>
      <td style="font-size:11px;color:var(--text-3)">${c.tool||'—'}</td>
...
python3 -c "
import re
with open('static/index.html') as f: h = f.read()
h = h.replace('async function loadDashboard()\n  loadDashboardCharts() {', 'async function loadDashboard() {')
h = h.replace('async function // already called below {', 'async function loadSSOProviders() {')
with open('static/index.html', 'w') as f: f.write(h)
print('JS OK')
"

cat > 'internal/api/handler/compliance.go' << 'GOEOF2'
package handler
...
# Add FedRAMP/CMMC routes to gateway
python3 << 'PYEOF2'
c = open("cmd/gateway/main.go").read()
if '/compliance/fedramp' not in c:
    c = c.replace(
        '\t\t// Compliance',
        '\t\t// Compliance\n\t\tr.Get("/api/v1/compliance/fedramp", complianceH.FedRAMP)\n\t\tr.Get("/api/v1/compliance/cmmc",    complianceH.CMMC)'
    )
    open("cmd/gateway/main.go", "w").write(c)
    print("FedRAMP routes added")
PYEOF2
...
sudo systemctl start vsp-gateway; sudo systemctl restart vsp-go-shell; sleep 3
...
curl -s -H "Authorization: Bearer $TOKEN" \
  http://localhost:8921/api/v1/compliance/fedramp \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'FedRAMP: {d[\"coverage_pct\"]}% coverage, {d[\"assessed\"]}/{d[\"total_controls\"]} controls')"

echo "--- CMMC"
curl -s -H "Authorization: Bearer $TOKEN" \
  http://localhost:8921/api/v1/compliance/cmmc \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'CMMC L2: {d[\"coverage_pct\"]}% coverage, {d[\"assessed\"]}/{d[\"total_practices\"]} practices')"

echo ""
echo "================================================================"
echo "  Dashboard v5: http://localhost:8922"
echo "  New tabs: SBOM | FedRAMP"
echo "  FedRAMP: 20 controls mapped to VSP tools"  
echo "  CMMC L2: 13 practices mapped to VSP tools"
echo "  SBOM: CycloneDX 1.5 download per run"
echo "================================================================"
