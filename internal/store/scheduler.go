...
func (db *DB) EnsureSchedulerTables(ctx context.Context) error {
	_, err := db.pool.Exec(ctx, `
		CREATE TABLE IF NOT EXISTS scan_schedules (
			...
			mode TEXT NOT NULL DEFAULT 'SAST', profile TEXT NOT NULL DEFAULT 'FAST',
			src TEXT NOT NULL DEFAULT '', url TEXT NOT NULL DEFAULT '',
			cron_expr TEXT NOT NULL DEFAULT '0 2 * * *', enabled BOOLEAN NOT NULL DEFAULT true,
			last_run_at TIMESTAMPTZ, next_run_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
		);
		CREATE TABLE IF NOT EXISTS drift_events (
			id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
			tenant_id UUID NOT NULL, schedule_id UUID,
			prev_posture TEXT NOT NULL DEFAULT '', new_posture TEXT NOT NULL DEFAULT '',
			prev_score INT NOT NULL DEFAULT 0, new_score INT NOT NULL DEFAULT 0,
			delta INT NOT NULL DEFAULT 0, rid TEXT NOT NULL DEFAULT '',
