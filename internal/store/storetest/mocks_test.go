...
func TestWebhookMock_ListEmpty(t *testing.T) {
	...
	if len(hooks) != 0 {
		t.Errorf("want 0, got %d", len(hooks))
	}
}
...
func TestWebhookMock_CreateAndList(t *testing.T) {
	...
	if len(hooks) != 1 {
		t.Fatalf("want 1, got %d", len(hooks))
	}
}
...
func TestWebhookMock_Touch(t *testing.T) {
	...
	if err := m.TouchSIEMWebhook(ctx, "h1"); err != nil {
		t.Fatal(err)
	}
	if m.TouchCalls != 1 {
		t.Errorf("want 1 touch call, got %d", m.TouchCalls)
	}
	if m.Hooks[0].FireCount != 1 {
		t.Errorf("want fire_count=1, got %d", m.Hooks[0].FireCount)
	}
	if m.Hooks[0].LastFired == nil {
		t.Error("want LastFired set")
	}
}
...
func TestWebhookMock_Delete(t *testing.T) {
	...
	if err := m.DeleteSIEMWebhook(ctx, "t1", "h1"); err != nil {
		t.Fatal(err)
	}
	hooks, _ := m.ListSIEMWebhooks(ctx, "t1")
	if len(hooks) != 1 {
		t.Errorf("want 1 after delete, got %d", len(hooks))
	}
	if hooks[0].ID != "h2" {
		t.Error("wrong hook remaining")
	}
}
...
func TestAuditMock_GetLastHash_Empty(t *testing.T) {
	...
	if hash != "" {
		t.Errorf("want empty hash for new tenant, got %q", hash)
	}
}
...
func TestAuditMock_Paged(t *testing.T) {
	...
	if len(page) != 2 {
		t.Errorf("want 2, got %d", len(page))
	}
	if count != 5 {
		t.Errorf("want total=5, got %d", count)
	}
}
...
func TestAuditMock_ActionFilter(t *testing.T) {
	...
	if len(page) != 1 {
		t.Errorf("want 1 filtered, got %d", len(page))
	}
	if count != 1 {
...
func TestCorrelatorMock_UpdateStatus(t *testing.T) {
	...
	if err := m.UpdateIncidentStatus(ctx, "t1", id, "resolved"); err != nil {
		t.Fatal(err)
	}
	...
	if inc.Status != "resolved" {
		t.Errorf("want resolved, got %s", inc.Status)
	}
}
...
func TestFindingMock_BatchInsert(t *testing.T) {
	...
	if err := m.InsertFindingsBatch(ctx, findings); err != nil {
		t.Fatal(err)
	}
...
func TestFindingMock_SeverityFilter(t *testing.T) {
	...
	if len(results) != 1 {
		t.Errorf("want 1 HIGH, got %d", len(results))
	}
}
...
func TestFindingMock_Summary(t *testing.T) {
	...
	if s.Critical != 1 {
		t.Errorf("critical: want 1, got %d", s.Critical)
	}
	if s.High != 2 {
		t.Errorf("high: want 2, got %d", s.High)
	}
}
