...
func buildReport(framework, version string, results []ControlResult, tenantID string) FrameworkReport {
	...
	coveragePct := 0
	passRate    := 0
	if total > 0    { coveragePct = assessed * 100 / total }
	if assessed > 0 { passRate    = passed   * 100 / assessed }
...
func (h *Compliance) OSCALAR(w http.ResponseWriter, r *http.Request) {
	...
	for _, ctrl := range results {
		state := "satisfied"
		if ctrl.Status == "fail" { state = "not-satisfied" }
		if ctrl.Status == "warn" { state = "satisfactory" }
		if ctrl.Status == "not_assessed" { state = "not-assessed" }
	...
	if err == nil && rows != nil {
		...
		for rows.Next() {
			...
			if rows.Scan(&id, &ruleID, &severity, &message, &tool, &cwe, &path, &createdAt) == nil {
				daysToResolve := 30
				if severity == "CRITICAL" { daysToResolve = 15 }
