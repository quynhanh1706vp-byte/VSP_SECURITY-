...
type VSPFinding struct {
	ID string `json:"id"`; Severity string `json:"severity"`; Title string `json:"title"`
	Description string `json:"description"`; CVE string `json:"cve,omitempty"`
	CWE string `json:"cwe,omitempty"`; Service string `json:"service,omitempty"`
	ControlID string `json:"control_id"`; CreatedAt time.Time `json:"created_at"`; Status string `json:"status"`
}
type SyncResult struct {
	Created int `json:"created"`; Updated int `json:"updated"`; Skipped int `json:"skipped"`
	NewIDs []string `json:"new_poam_ids"`; SyncedAt time.Time `json:"synced_at"`
}
func mapFindingCtrl(f VSPFinding) string {
	t := strings.ToLower(f.Title + " " + f.Description)
	switch {
	case strings.Contains(t,"sql")||strings.Contains(t,"injection"): return "SI-10"
	case strings.Contains(t,"xss")||strings.Contains(t,"cross-site"): return "SI-10"
	case strings.Contains(t,"auth")||strings.Contains(t,"mfa"): return "IA-2"
	case strings.Contains(t,"encrypt")||strings.Contains(t,"tls"): return "SC-8"
	case strings.Contains(t,"patch")||strings.Contains(t,"cve"): return "SI-2"
	case strings.Contains(t,"access")||strings.Contains(t,"privilege"): return "AC-6"
	case strings.Contains(t,"audit")||strings.Contains(t,"log"): return "AU-2"
	case strings.Contains(t,"config"): return "CM-6"
	case strings.Contains(t,"supply")||strings.Contains(t,"depend"): return "SA-12"
	case strings.Contains(t,"network")||strings.Contains(t,"firewall"): return "SC-7"
	case strings.Contains(t,"secret")||strings.Contains(t,"key")||strings.Contains(t,"credential"): return "IA-5"
	case strings.Contains(t,"acl")||strings.Contains(t,"bucket")||strings.Contains(t,"s3"): return "AC-6"
	default: return "SI-3"
	}
}
...
func syncFindingsToPOAM(findings []VSPFinding) SyncResult {
	...
	rmfStore.mu.Lock(); defer rmfStore.mu.Unlock()
	pkg := rmfStore.packages["VSP-DOD-2025-001"]
	if pkg == nil { return result }
	...
	for _,f := range findings {
		fid := "FIND-VSP-"+f.ID; if len(fid)>30{fid=fid[:30]}
		if existing[fid] || existing[f.Title] { result.Skipped++; continue }
		due := time.Now()
		if f.Severity=="CRITICAL"{due=due.AddDate(0,0,7)}else{due=due.AddDate(0,0,30)}
		p := POAMItem{
		...
		pkg.POAMItems=append(pkg.POAMItems,p); existing[fid]=true
		result.Created++; result.NewIDs=append(result.NewIDs,p.ID)
	}
	pkg.UpdatedAt=time.Now(); return result
}
	...
	if p4SQLDB != nil {
		...
		if err == nil {
			...
			for rows.Next() {
				...
				if len(f.Title) > 80 { f.Title = f.Title[:80] }
				// rule_id có thể là CVE
				if strings.HasPrefix(ruleID, "CVE-") { f.CVE = ruleID }
				f.Status = "open"
...
func handleFindingsSync(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type","application/json"); w.Header().Set("Vary","Origin")
...
type RenewalItem struct {ID string `json:"id"`; Task string `json:"task"`; Done bool `json:"done"`; DueWeeks int `json:"due_weeks_before_expiry"`}
func handleATOExpiry(w http.ResponseWriter, r *http.Request) {
	...
	if pkg==nil{http.Error(w,"no ATO",404);return}
	...
	if p4SQLDB != nil {
		...
		if err == nil {
			if ad != nil { pkg.AuthorizationDate = ad }
			if ed != nil { pkg.ExpirationDate = ed }
		}
	}
	...
	if pkg.ExpirationDate!=nil {
		...
		if days<0{level="expired"}else if days<90{level="critical"}else if days<180{level="warning"}
	}
...
type SBOMComp struct{Name string `json:"name"`; Version string `json:"version"`; Type string `json:"type"`; License string `json:"license"`; CVEs int `json:"cves"`; Sev string `json:"severity,omitempty"`; NTIA bool `json:"ntia_compliant"`}
func handleSBOMView(w http.ResponseWriter, r *http.Request) {
	...
	high:=0; for _,c:=range comps{if c.Sev=="HIGH"{high++}}
	violations:=[]string{}; if high>0{violations=append(violations,fmt.Sprintf("%d HIGH CVEs require remediation within 30 days",high))}
